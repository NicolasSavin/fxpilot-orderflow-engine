...
def _bias(current_delta: float, current_cumdelta: float, slope: str) -> str:
    if slope == "rising" and current_delta > 0 and current_cumdelta > 0:
        return "bullish"
    if slope == "falling" and current_delta < 0 and current_cumdelta < 0:
        return "bearish"
    return "neutral"
...
class CumDeltaEngine:
    ...
    def _result(self, symbol: str) -> CumDeltaResult:
        ...
        return CumDeltaResult(
            ...
            bias=_bias(latest.delta, latest.cumdelta, slope),
        )
